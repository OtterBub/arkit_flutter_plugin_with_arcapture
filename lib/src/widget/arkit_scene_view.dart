...
class ARKitController {
  ...
  Future<void> allClearObject() {
    return _channel.invokeMethod('allClearObject');
  }
  ...
  void updateFaceGeometry(ARKitNode node, String fromAnchorId) {
    _channel.invokeMethod<void>(
        'updateFaceGeometry', _getHandlerParams(node, 'geometry', <String, dynamic>{'fromAnchorId': fromAnchorId}));
  }

  Future<Vector3?> projectPoint(Vector3 point) async {
    final projectPoint =
        await _channel.invokeListMethod<double>('projectPoint', {'point': _vector3Converter.toJson(point)});
  ...
  void _subsribeToChanges(ARKitNode node) {
    ...
    if (node.light != null) {
      node.light!.intensity
          .addListener(() => _updateSingleProperty(node, 'intensity', node.light!.intensity.value, 'light'));
    }
  }
  ...
  void _subscribeToConeGeometry(ARKitNode node) {
    ...
    cone.bottomRadius
        .addListener(() => _updateSingleProperty(node, 'bottomRadius', cone.bottomRadius.value, 'geometry'));
  ...
  void _handleTransformationChanged(ARKitNode node) {
    _channel.invokeMethod<void>('transformationChanged',
        _getHandlerParams(node, 'transformation', _matrixValueNotifierConverter.toJson(node.transformNotifier)));
  }

  void _handleIsHiddenChanged(ARKitNode node) {
    _channel.invokeMethod<void>(
        'isHiddenChanged', _getHandlerParams(node, 'isHidden', _boolConverter.toJson(node.isHidden)));
  }
