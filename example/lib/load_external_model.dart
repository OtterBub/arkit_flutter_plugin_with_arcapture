...
class LoadExternal3DModel extends StatefulWidget {
  @override
  _LoadExternal3DModelState createState() => _LoadExternal3DModelState();
}

class _LoadExternal3DModelState extends State<LoadExternal3DModel> {
  ...
  void onARKitViewCreated(ARKitController arkitController) {
    ...
    this.arkitController.add(
          ARKitReferenceNode(
            ...
            position: vector.Vector3(0.0, 0.1, -0.5),
            name: 'dash',
          ),
        );

    final otherNode = ARKitReferenceNode(
        url: 'models.scnassets/arrow_skpark.dae',
        scale: vector.Vector3.all(0.1),
        position: vector.Vector3(0.0, 0.2, -0.5),
        name: 'arrow');
    this.arkitController.add(otherNode);
  }

  ARKitNode _createSphere() => ARKitNode(
        geometry: ARKitSphere(materials: _createRandomColorMaterial(), radius: 0.04),
  ...
  ARKitNode _createBox() => ARKitNode(
        geometry: ARKitBox(
            width: 0.06, height: 0.06, length: 0.06, chamferRadius: 0.01, materials: _createRandomColorMaterial()),
        position: vector.Vector3(-0.1, 0, -0.5),
      );

  ARKitNode _createCylinder() => ARKitNode(
        geometry: ARKitCylinder(radius: 0.05, height: 0.09, materials: _createRandomColorMaterial()),
        position: vector.Vector3(-0.1, 0.1, -0.5),
      );

  ARKitNode _createCone() => ARKitNode(
        geometry: ARKitCone(topRadius: 0, bottomRadius: 0.05, height: 0.09, materials: _createRandomColorMaterial()),
        position: vector.Vector3(0, -0.1, -0.5),
      );

  ARKitNode _createPyramid() => ARKitNode(
        geometry: ARKitPyramid(width: 0.09, height: 0.09, length: 0.09, materials: _createRandomColorMaterial()),
        position: vector.Vector3(0, -0.05, -0.5),
      );

  ARKitNode _createTube() => ARKitNode(
        geometry:
            ARKitTube(innerRadius: 0.03, outerRadius: 0.05, height: 0.09, materials: _createRandomColorMaterial()),
        position: vector.Vector3(0.1, 0.1, -0.5),
      );

  ARKitNode _createTorus() => ARKitNode(
        geometry: ARKitTorus(ringRadius: 0.04, pipeRadius: 0.02, materials: _createRandomColorMaterial()),
        position: vector.Vector3(0.1, -0.1, -0.5),
      );

  ARKitNode _createCapsule() => ARKitNode(
        geometry: ARKitCapsule(capRadius: 0.02, height: 0.06, materials: _createRandomColorMaterial()),
        position: vector.Vector3(0.1, 0, -0.5),
      );
